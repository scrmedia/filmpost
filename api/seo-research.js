...
export default async function handler(req, res) {
  ...
  const { venue, location = 'United Kingdom' } = req.body || {};
  ...
  try {
    const [suggest, serp] = await Promise.all([
      dfs('/dataforseo_labs/google/keyword_suggestions/live', {
        ...base, keyword: venue, limit: 100,
        filters: [['keyword_info.search_volume', '>', 0]],
        order_by: ['keyword_info.search_volume,desc'],
      }).catch(() => ({})), // venue names often have no suggestion data; SERP still useful
      dfs('/serp/google/organic/live/advanced', { ...base, keyword: fallback, depth: 20 }),
    ]);

    const keywords = (suggest.items || []).map(i => ({
      keyword: i.keyword,
      volume: i.keyword_info?.search_volume || 0,
      kd: i.keyword_properties?.keyword_difficulty ?? null,
    }));
    const v = norm(venue);
    const weddingKeywords = keywords
      .filter(k => norm(k.keyword).includes(v) && /wedding|videograph|film/.test(k.keyword))
      // equal volume is common (Google groups variants): prefer "venue ..." word order, then shortest
      .sort((a, b) => b.volume - a.volume
        || norm(b.keyword).startsWith(v) - norm(a.keyword).startsWith(v)
        || a.keyword.length - b.keyword.length);
    // ponytail: "highest volume wedding keyword" heuristic; let Claude weigh KD/intent if picks look off
    const primary = weddingKeywords[0]?.keyword || fallback;
    ...
    res.status(200).json({
      ...
      secondaryKeyword: primary === fallback ? null : fallback,
      keywords: weddingKeywords.slice(0, 15),
